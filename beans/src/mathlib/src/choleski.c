...
int Lower_Triangular_Inverse_f(float *L, int n);
...
int Choleski_LU_Decomposition_f(float *A, int n)
{
   int i, k, p;
...
int Choleski_LU_Inverse_f(float *LU, int n)
{
   int i, j, k;
   float *p_i, *p_j, *p_k;
   double sum;

   if ( Lower_Triangular_Inverse_f(LU, n) < 0 ) return -1;
